...
`fixtures` vs `corpus`: use `fixtures` when the subject is the trigger payload
or the S3 round trip and record content is noise; use `corpus` when the subject
_is_ the record (dot paths, unresolvable leaves, byte-exact survival). The
corpus lives in `core` — not here — because core's own integration tests need
it and core must not dev-depend on this crate; that would pull the AWS SDK into
the dependency graph of the one crate defined by not having it.
