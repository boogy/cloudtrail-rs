...
> **SQS: a message that is not an S3 notification now fails.** A body that is
> valid JSON but carries no `Records` array used to decode to zero objects and
> ack clean. It is now a decode error, so the message is redelivered and lands
> in the DLQ. That is the correct outcome for a real S3 notification whose shape
> changed under us, but it also means an **SNS `SubscriptionConfirmation`
