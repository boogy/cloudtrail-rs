...
| 19 | Docs and examples | 18 | done | 0da43d6 | Sonnet subagent, orchestrator-verified. README.md (459 lines): arch + mermaid, 4 trigger topologies, full CT_* env table, per-role IAM (cross-checked vs actual SDK calls), `cargo lambda build --release --arm64`, dry-run rollout, SQS ReportBatchItemFailures data-loss warning, cold-start section, always-bucket guidance, YAML quoting trap, all 3 CLI subcommands incl. new filter folder/s3:// batch mode w/ local→local/s3 examples, MiniStack run steps. No new test (equivalent already exists: cli.rs validate_example_ruleset...). examples/*.yaml verified byte-identical to fixtures (no drift). Gates green. **PLAN COMPLETE (00–19 all done).** |
